 ...
 *
 * This host is a ZONE: client-routed inside `/my-account/*` (docs/navigation-zones.md).
 * What the server does is therefore narrower than on the storefront, and the boundary is
 * the same one D12 draws everywhere else:
 *
 *   SERVER  chrome, the account frame, and a correctly-sized skeleton for the route.
 *           Identical for every visitor, so the document stays shared-cacheable and the
 *           first paint has a real element instead of an empty box.
 *
 *   CLIENT  everything per-user, and every navigation after the first.
 *
 * It never server-renders an order. Doing so would make every response user-specific,
 * destroy shared caching, and put personalization on the TTFB path — the exact mistake
 * already reversed once for the cart.
 */
...
export interface RenderInput {
   ...
   *
   * Gating happens in the server before render is called — a redirect is cheaper than a
   * render, and it keeps "who may see this" in one place rather than scattered through
   * components.
   */
   ...
   * never shared between two people — so there is nothing to protect by pretending not to
   * know. Rendering the name server-side puts it in the first paint instead of a beat later,
   * and it tells the SPA who it is serving without the client re-deriving it from a cookie.
   *
...
export async function renderApp(input: RenderInput): Promise<RenderOutput> {
  ...
  // The skeleton the client will replace. Same markup the client renders while loading, so
  // the handover is invisible and costs no layout shift.
  ...
  const appHtml = renderToString(
    <SlotProvider slots={slots} onUse={(n) => usedSlots.add(n)}>
      <div className="flex min-h-screen flex-col bg-page">
        {Chrome ? <Chrome.Header host="my-account" viewer={input.viewer ?? null} /> : null}
        <main id="main" className="flex-1">
          {input.login ? (
            <Login next={input.login.next} error={input.login.error} email={input.login.email} />
          ) : (
            /* The client takes over this whole element: the sidebar's active state changes
               on every soft navigation, so the frame belongs to the application, not to the
               server render that seeded it. */
            <div id="account-frame">
              <Frame activeId={routeId ?? 'account.overview'} viewer={input.viewer ?? null}>
                {skeleton ? (
                  skeleton()
                ) : (
   ...
   * storefront uses. Without it the shared header's signed-in label never attaches HERE,
   * which is the one page where a visitor most expects to see their own name.
   */
  const behaviors = [
  ...
  const html =
    ...
    // Authenticated, per-user, and of no use to a crawler. Said explicitly rather than
    // left to a robots.txt that nobody reviews.
