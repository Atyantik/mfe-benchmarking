<script lang="ts">
  ...
  import { Button, Container } from '@mf-eval/design-svelte';
  import { ACCOUNT, AUTH } from '@mf-eval/contracts/testids';

  /**
   * The account frame — sidebar and page shell.
   *
   * SERVER-RENDERED and never hydrated, exactly like a document page. It is identical for every
   * visitor, so it is shared-cacheable, and it gives the first paint a real element instead of
   * an empty box.
   *
  ...
  let { activeId, children, viewer = null }: {
    activeId: string;
    children: Snippet;
...
<Container class="py-8">
  <div class="grid gap-8 lg:grid-cols-[15rem_1fr]">
    <aside>
      <h1 class="mb-1 text-[length:var(--fs-xl)] font-semibold tracking-tight text-ink-900">My account</h1>
      {#if viewer}
        <p class="mb-4 text-[length:var(--fs-sm)] text-ink-500" data-testid={ACCOUNT.viewer}>
      ...
      <!-- A POST, not a link. A GET that ends a session is signed out by any prefetcher, link
           scanner or antivirus proxy that touches the page. -->
      <form method="post" action="/logout" class="mt-6">
        <Button type="submit" tone="ghost" size="sm" data-testid={AUTH.signOut}>Sign out</Button>
      </form>
    </aside>
    <!-- The one region the client owns. Everything inside is per-user, so the server renders a
         correctly-sized skeleton here and the client replaces it. -->
